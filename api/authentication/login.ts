...
import {openUserDropDown} from "../user/openUserDropdown";

let token;
let auth;

function login(event: Event, token: any): void {


    if (token != null) {//if user has logged in before
        var ajax:XMLHttpRequest = new XMLHttpRequest(); //database transaction request ( without sending data)
        var method:string = "GET";
        var url: string = "user/get-user-information.php";  //PHP file
        ...
        ajax.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                document.getElementById("settingsBtn").addEventListener("click", function () { userSetting() });
                ...
                (document.getElementById("uNameForPet") as HTMLFormElement).value  = data.uName;
